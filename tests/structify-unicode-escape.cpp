...
struct UStruct
{
  ...
  STFY_OBJ(euro, cjk, oslash, grin, ascii);
};
...
TEST_CASE("unicode_escape_decoding", "[structify][utf-8]")
{
  STFY::ParseContext context(unicode_json);
  UStruct s;
...
TEST_CASE("unicode_escape_roundtrip", "[structify][utf-8]")
{
  STFY::ParseContext context(unicode_json);
  UStruct s;
  REQUIRE(context.parseTo(s) == STFY::Error::NoError);

  std::string serialized = STFY::serializeStruct(s);

  STFY::ParseContext context2(serialized);
  UStruct s2;
  REQUIRE(context2.parseTo(s2) == STFY::Error::NoError);
