...
struct ExecuteOneData
{
  ...
  std::string prop2;
  std::string prop3;
  STFY_OBJECT(STFY_MEMBER(prop1), STFY_MEMBER(prop2), STFY_MEMBER(prop3));
};
struct ExecuteTwoData
{
  std::string first_prop;
  STFY_OBJECT(STFY_MEMBER(first_prop));
};
struct ExecuterTwoReturn
{
  ...
  STFY_OBJECT(STFY_MEMBER(string_data), STFY_MEMBER(value), STFY_MEMBER(values));
};
struct Executor
{
  ...
  STFY_FUNCTION_CONTAINER(STFY_FUNCTION(execute_one), STFY_FUNCTION(execute_two));
};
...
TEST_CASE("test_function_error_simple", "[function][error]")
{
  Executor executor;
  std::string json_out;
  STFY::DefaultCallFunctionContext context(json_data, json_out);
