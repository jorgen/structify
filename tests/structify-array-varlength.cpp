...
struct JsonObject1
{
  STFY::ArrayVariableContent<int, 10> vec;
  STFY_OBJ(vec);
};

struct JsonObject2
{
  STFY::ArrayVariableContent<int, 2> vec;
  STFY_OBJ(vec);
};

TEST_CASE("array_variable_content", "[structify][array]")
{
  STFY::ParseContext context(json_data);
  JsonObject1 object;
  REQUIRE(context.parseTo(object) == STFY::Error::NoError);
...
TEST_CASE("array_variable_content_fail", "[structify][array]")
{
  STFY::ParseContext context(json_data);
  JsonObject2 object;
  REQUIRE(context.parseTo(object) != STFY::Error::NoError);
