...
struct SubObject_t
{
  ...
  STFY_OBJ(SimpleMember, MoreValues);
};
struct ComplexFields2_t
{
  ...
  STFY_OBJ(SimpleMember, ArrayOfValues, SubObject);
};
...
struct Root_t
{
  ...
  STFY_OBJ(Field1, Field2, Field3, Field4, ComplexFields, ComplexFields2);
};
...
void verify_map_meta(const STFY::Map &map)
{
  for (auto &meta : map.meta)
  {
    bool is_complex = (map.tokens.data[meta.position].value_type == STFY::Type::ObjectStart ||
            map.tokens.data[meta.position].value_type == STFY::Type::ArrayStart);
    REQUIRE(is_complex);
  }

  auto new_meta = STFY::metaForTokens(map.tokens);
...
TEST_CASE("polymorphic_map_basic", "structify")
{
  STFY::Map map;
  STFY::ParseContext pc(json, sizeof(json), map);
  ...
  REQUIRE(it != map.end());
  REQUIRE(it->value_type == STFY::Type::Bool);
  ...
  REQUIRE(map.castToType("ComplexFields", pc, newComplexField2) == STFY::Error::NoError);
...
TEST_CASE("clean_map", "structify, map")
{
  STFY::Map map;
  STFY::ParseContext pc;
...
TEST_CASE("to_clean_map", "structify, map")
{
...
struct ParsedJsonMap {
  ...
  struct {
    ...
    int innerKey2;
    STFY_OBJ(innerKey1, innerKey2);
  } mapKey;
  STFY_OBJ(regularKey, mapKey);
};


TEST_CASE("nested_map", "structify, map")
{
  STFY::Map nestedMap;
  STFY::ParseContext parseContext;
  ...
  std::string output_json = STFY::serializeStruct(outerMap);
  STFY::ParseContext parseContext2(output_json.data(), output_json.size(), parsed_json);
  REQUIRE(parseContext2.error == STFY::Error::NoError);
...
//TEST_CASE("fail_to_compile", "structify, map")
//{
//  STFY::Map map;
//  STFY::ParseContext pc;
//  int foo = 44;
