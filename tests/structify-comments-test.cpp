...
namespace structify_comments_test
{
...
struct DatabaseConfig
{
  ...
  STFY_OBJECT(STFY_MEMBER(host), STFY_MEMBER(port), STFY_MEMBER(ssl_enabled));
};
...
struct FeatureFlags
{
  ...
  STFY_OBJECT(STFY_MEMBER(beta_ui), STFY_MEMBER(new_parser));
};
...
struct AppConfig
{
  ...
  STFY_OBJECT(STFY_MEMBER(app_name), STFY_MEMBER(version), STFY_MEMBER(database),
            STFY_MEMBER(preferences), STFY_MEMBER(features));
};

TEST_CASE("parse_struct_with_comments", "[structify][comments]")
{
  STFY::ParseContext context(json_with_comments);
...
struct DeepInner
{
  ...
  STFY_OBJECT(STFY_MEMBER(value), STFY_MEMBER(name));
};
...
struct NestedCommentStruct
{
  ...
  STFY_OBJECT(STFY_MEMBER(outer));
};

TEST_CASE("parse_nested_struct_with_comments", "[structify][comments]")
{
  STFY::ParseContext context(nested_comments_json);
  context.tokenizer.allowComments(true);
...
struct MessageData
{
  ...
  STFY_OBJECT(STFY_MEMBER(message), STFY_MEMBER(priority));
};
...
struct GenericContainer
{
  ...
  STFY_OBJECT(STFY_MEMBER(container_id), STFY_MEMBER(data));
};

TEST_CASE("parse_template_struct_with_comments", "[structify][comments]")
{
  STFY::ParseContext context(template_json_with_comments);
  context.tokenizer.allowComments(true);
...
struct BaseClass
{
  ...
  STFY_OBJECT(STFY_MEMBER(base_id), STFY_MEMBER(base_name));
};
...
struct DerivedClass : public BaseClass
{
  ...
  STFY_OBJECT_WITH_SUPER(STFY_SUPER_CLASSES(STFY_SUPER_CLASS(BaseClass)),
                       STFY_MEMBER(derived_value), STFY_MEMBER(derived_flag));
};

TEST_CASE("parse_inheritance_struct_with_comments", "[structify][comments]")
{
  STFY::ParseContext context(inheritance_json_with_comments);
  context.tokenizer.allowComments(true);
...
{
  std::string theme;
  STFY::OptionalChecked<bool> notifications;

  STFY_OBJECT(STFY_MEMBER(theme), STFY_MEMBER(notifications));
};
...
struct User
{
  std::string name;
  int age = 0;
  STFY::OptionalChecked<std::string> email;
  STFY::OptionalChecked<std::string> phone;
  UserSettings settings;

  STFY_OBJECT(STFY_MEMBER(name), STFY_MEMBER(age), STFY_MEMBER(email),
            STFY_MEMBER(phone), STFY_MEMBER(settings));
};

TEST_CASE("parse_optional_members_with_comments", "[structify][comments]")
{
  STFY::ParseContext context(optional_json_with_comments);
  context.tokenizer.allowComments(true);
...
struct SimpleItem
{
  int id = 0;
  std::string name;
  STFY::OptionalChecked<bool> active;

  STFY_OBJECT(STFY_MEMBER(id), STFY_MEMBER(name), STFY_MEMBER(active));
};

TEST_CASE("parse_multi_top_level_with_comments", "[structify][comments]")
{
  STFY::ParseContext context(multi_level_json_comments);
  context.tokenizer.allowComments(true);
...
struct Employee
{
  ...
  STFY_OBJECT(STFY_MEMBER(name), STFY_MEMBER(department), STFY_MEMBER(salary), STFY_MEMBER(skills));
};
...
struct Company
{
  ...
  STFY_OBJECT(STFY_MEMBER(employees));
};

TEST_CASE("parse_array_of_structs_with_comments", "[structify][comments]")
{
  STFY::ParseContext context(array_json_with_comments);
  context.tokenizer.allowComments(true);
...
TEST_CASE("serialization_round_trip_with_comments", "[structify][comments]")
{
  // Parse with comments
  STFY::ParseContext context(json_with_comments);
  context.tokenizer.allowComments(true);
  ...
  REQUIRE(parse_error == STFY::Error::NoError);

  // Serialize back to JSON (without comments)
  std::string serialized = STFY::serializeStruct(original);

  // Parse the serialized version (no comments)
  STFY::ParseContext new_context(serialized);
  ...
  REQUIRE(reparse_error == STFY::Error::NoError);
...
TEST_CASE("error_when_comments_disabled", "[structify][comments]")
{
  STFY::ParseContext context(json_with_comments);
...
{
  std::string value;
  std::vector<STFY::JsonObjectOrArray> array;

  STFY_OBJECT(STFY_MEMBER(value), STFY_MEMBER(array));
};
...
struct Level3
{
  ...
  STFY_OBJECT(STFY_MEMBER(level4));
};
...
struct Level2
{
  ...
  STFY_OBJECT(STFY_MEMBER(level3));
};
...
struct Level1
{
  ...
  STFY_OBJECT(STFY_MEMBER(level2));
};
...
struct DeeplyNested
{
  ...
  STFY_OBJECT(STFY_MEMBER(level1));
};

TEST_CASE("parse_deeply_nested_with_comments", "[structify][comments]")
{
  STFY::ParseContext context(deeply_nested_comments);
  context.tokenizer.allowComments(true);
