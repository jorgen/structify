...
struct JsonData
{
  ...
  sys_tp_t tp_3_us;
  hpc_tp_t tp_4_ns;
  STFY_OBJ(tp_0_s, tp_0_ms, tp_0_us, tp_0_ns, tp_1_s, tp_2_ms, tp_3_us, tp_4_ns);
};
...
TEST_CASE("time_point", "structify")
{
  JsonData dataStruct;
  STFY::ParseContext parseContext(json);
  REQUIRE(parseContext.parseTo(dataStruct) == STFY::Error::NoError);
  ...
  std::string genjson = STFY::serializeStruct(dataStruct);
  JsonData dataStruct2;
  REQUIRE(memcmp(&dataStruct2, &dataStruct, sizeof(JsonData)));
  STFY::ParseContext parseContext2(genjson);
  REQUIRE(parseContext2.parseTo(dataStruct2) == STFY::Error::NoError);
  ...
  STFY::ParseContext parseContext3(genjson);
  REQUIRE(parseContext3.parseTo(dataStruct3) == STFY::Error::NoError);
