...
void check_parse_nullterminated_string(const std::string jsonWithNullTerminator)
{
  ...
  STFY::ParseContext pc(jsonWithNullTerminator);
...
TEST_CASE("check_parse_nullterminated_string", "[structify]")
{
...
TEST_CASE("check_serialize_nullterminated_struct", "[structify]")
{
  NullTerminatorStruct nullStruct;
  nullStruct.data = create_string_with_nullterminators();
  std::string json = STFY::serializeStruct(nullStruct);
