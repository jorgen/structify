...
{
    STFY::TreeBuilder tree_builder;
    auto created = tree_builder.build(json_data2,sizeof(json_data2));
    STFY::Node *root = created.first;
    assert(root);

    check_json_tree_from_json_data2(root);

    STFY::SerializerOptions printerOption(false);
    char buffer[4096];
    memset(buffer,'\0', 4096);
    STFY::TreeSerializer serializer(buffer,4096);
...
static int check_json_tree_printer_pretty()
{
    ...
    STFY::SerializerOptions printerOption(true);
    char buffer[4096];
    memset(buffer,'\0', 4096);
    STFY::TreeSerializer serializer(buffer,4096);
    assert(serializer.serialize(root->asObjectNode()));
...
static int check_multiple_print_buffers()
{
    ...
    STFY::TreeSerializer serializer;
    serializer.setOptions(STFY::SerializerOptions(true));
    ...
    serializer = STFY::TreeSerializer();
    serializer.setOptions(STFY::SerializerOptions(true));
...
static void add_buffer_func(STFY::Serializer *printHandler)
{
