...
TEST_CASE("cbor_simple_map", "[cbor][tokenizer]")
{
  ...
  error = tokenizer.nextToken(token);
  REQUIRE(error == JS::Error::NoError);
  REQUIRE(assert_token(token, JS::Type::Ascii, "a", JS::Type::Number, "1") == 0);

  error = tokenizer.nextToken(token);
  REQUIRE(error == JS::Error::NoError);
  REQUIRE(assert_token(token, JS::Type::Ascii, "b", JS::Type::Number, "2") == 0);
...
TEST_CASE("cbor_nested_map_with_array", "[cbor][tokenizer]")
{
  ...
  error = tokenizer.nextToken(token);
  REQUIRE(error == JS::Error::NoError);
  REQUIRE(assert_token(token, JS::Type::Ascii, "name", JS::Type::String, "John") == 0);
...
TEST_CASE("cbor_map_with_string_values", "[cbor][tokenizer]")
{
  ...
  error = tokenizer.nextToken(token);
  REQUIRE(error == JS::Error::NoError);
  REQUIRE(assert_token(token, JS::Type::Ascii, "key", JS::Type::String, "value") == 0);
