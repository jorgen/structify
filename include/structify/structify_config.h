...
/*! \mainpage structify
 *
 * structify is a set of classes meant for simple and efficient parse,
 * tokenize and validate json.
 *
 * structify support parsing json into a stream of tokens using the \ref
 * tokenizer "STFY::Tokenizer" api, or parsing json into c++ structures using the
 * \ref js_struct "STFY_OBJECT" api.
 */

/*! \page tokenizer Parsing json using STFY::Tokenizer
 *
 * Tokenizing json STFY::Tokenizer can be used to extract tokens
 ...
 * requires the application code to keep some extra state. structify also has
 ...
 *
 * This example show very basic usage of how STFY::Tokenizer can be used
 */
 ...
 *
 * structify makes it very easy to put your json data into c++ structures or
 ...
 * \endcode
 *
 * The STFY_OBJECT call inside the JsonData struct will create a nested struct
 ...
 * structify can then use this runtime information to populate the struct.
 *
 * Populating the struct is done by first creating a STFY::ParseContext. The
 * STFY::ParseContext contains a STFY::Tokenizer. This tokenizer is what the actual
 * state holder for the parsing. If allowing using '\n' instead of ',' to
 * seperate object and array elements, then this should be set on the
 * STFY::Tokenizer.
 *
 * Since types will dictate the schema of the input json, the STFY::ParseContext
...
#define STFY_STD_UNORDERED_MAP 1
#endif
#ifdef STFY_STD_UNORDERED_MAP
#include <unordered_map>
#endif

#ifndef STFY_STD_OPTIONAL
#if defined(__APPLE__)
#if __clang_major__ > 9 && __cplusplus >= 201703L
#define STFY_STD_OPTIONAL 1
#endif
...
#ifndef STFY_IF_CONSTEXPR
#if __cpp_if_constexpr
#define STFY_IF_CONSTEXPR(exp) if constexpr (exp)
#elif defined(_MSC_VER)
#define STFY_IF_CONSTEXPR(exp) __pragma(warning(push)) __pragma(warning(disable : 4127)) if (exp) __pragma(warning(pop))
#else
#define STFY_IF_CONSTEXPR(exp) if (exp)
#endif
#endif

#if STFY_NO_NODISCARD
#define STFY_NODISCARD
#else
#if __cplusplus >= 201703L
#define STFY_NODISCARD [[nodiscard]]
#else
#define STFY_NODISCARD
#endif
#endif
...
#error min or max macro is defined. Make sure these are not defined before including structify.h.\
 Use "#define NOMINMAX 1" before including Windows.h
#endif

#define STFY_UNUSED(x) (void)(x)

#ifndef STFY_DISABLE_SIMD
#if defined(__SSE2__) || defined(_M_X64) || (defined(_M_IX86_FP) && _M_IX86_FP >= 2)
#define STRUCTIFY_HAS_SSE2 1
#include <emmintrin.h>
#endif

#if defined(__SSE4_2__) || (defined(_MSC_VER) && defined(__AVX__))
#define STRUCTIFY_HAS_SSE4_2 1
#include <nmmintrin.h>
#endif

#if defined(__AVX2__)
#define STRUCTIFY_HAS_AVX2 1
#include <immintrin.h>
#endif

#if defined(__BMI__) || defined(__BMI2__)
#define STRUCTIFY_HAS_BMI 1
#include <immintrin.h>
#endif

#if defined(__aarch64__) || defined(_M_ARM64) || defined(__ARM_NEON)
#define STRUCTIFY_HAS_NEON 1
#include <arm_neon.h>
#endif
#endif
...
#define STRUCTIFY_LIKELY(x) __builtin_expect(!!(x), 1)
#define STRUCTIFY_UNLIKELY(x) __builtin_expect(!!(x), 0)
#define STRUCTIFY_FORCE_INLINE __attribute__((always_inline)) inline
#define STRUCTIFY_COLD __attribute__((cold))
#define STRUCTIFY_HOT __attribute__((hot))
#define STRUCTIFY_RESTRICT __restrict__
#elif defined(_MSC_VER)
#define STRUCTIFY_LIKELY(x) (x)
#define STRUCTIFY_UNLIKELY(x) (x)
#define STRUCTIFY_FORCE_INLINE __forceinline
#define STRUCTIFY_COLD
#define STRUCTIFY_HOT
#define STRUCTIFY_RESTRICT __restrict
#else
#define STRUCTIFY_LIKELY(x) (x)
#define STRUCTIFY_UNLIKELY(x) (x)
#define STRUCTIFY_FORCE_INLINE inline
#define STRUCTIFY_COLD
#define STRUCTIFY_HOT
