...
/*! \mainpage structify_diff
 *
 * structify_diff is an extension to structify to efficiently diff json by
 * using structify itself.
 */

#ifndef STRUCTIFY_DIFF_H
#define STRUCTIFY_DIFF_H
...
struct DiffTokens
{
    ...
    void generateTokens(const char* json, size_t size)
    {
        ...
        {
            e = tokenizer.nextTokens(&token, 1, count);
            if (e == Error::NoError)
    ...
    {
        assert(startPos < size());
        assert(tokens.data[startPos].value_type == STFY::Type::ObjectStart);
    ...
    {
        assert(startPos < size());
        assert(tokens.data[startPos].value_type == STFY::Type::ArrayStart);
        const Token &token = baseTokens.tokens.data[basePos];
...
namespace Internal
{
    namespace Diff
    {
        ...
        inline void diffBooleanValues(const Token &baseToken, const Token &diffToken, DiffTokens &diff, const size_t diffPos)
        {
            ...
            assert(dummyContext.error == STFY::Error::NoError);
                ...
                else
                {
                    ...
                    else if (diffToken.value_type == Type::ArrayEnd)
                    {
                        while (true)
                        {
                            ...
                            if (bToken.value_type == STFY::Type::ArrayEnd)
                                break;
